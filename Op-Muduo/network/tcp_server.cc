...
void TcpServer::newConnectionArrived(Socket &sock, InternetAddress &address) {
  ...
  TcpConnectionPtr new_connection =
      std::make_shared<TcpConnection>(owner_event_loop_, sock, address);
...
void TcpServer::removeConnection(TcpConnectionPtr connection) {
  ...
  // check removed
  assert(remove_num == 1);
  // must run in loop, delay the destruction of connection until the channel
  // callback of connection finish.
  owner_event_loop_->queueInLoop([connection]() { connection->destroyConnection(); });
}
